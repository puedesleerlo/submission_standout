...
For either evaluator, reported fractions are **passes in simulated trials, not estimated real acceptance probabilities**. A comparison of fixed documents does not establish generative policy quality across opportunities. Reusing practice environments can overfit; independent holdout opportunities and policy promotion remain future work. A replay verifies deterministic allocation from saved assessments, not reproducibility of a fresh LLM judgment.
