...
const navigation: { id: Page; name: string; icon: typeof FileText }[] = [
  ...
  { id: 'library', name: 'Reusable banks', icon: BookOpen },
...
function Workbench() {
  ...
  const [workspace, setWorkspace] = useState<Workspace | null>(null); const [page, setPage] = useState<Page>(() => {
    const view = new URLSearchParams(location.search).get('view');
    return navigation.find(item => item.id === view)?.id || 'studio';
  });
  ...
  useEffect(() => {
    ...
    api<Project[]>('/projects').then(items => {
      setProjects(items);
      const saved = new URLSearchParams(location.search).get('project') || localStorage.getItem('standout.project');
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>, kind: string) {
    ...
    await action(kind, async () => {
      if (kind === 'project') {
        const item = await post<Project>('/projects', { name: text('name'), institution: text('institution'), posting: text('posting'), deadline: text('deadline') || null, domain: text('domain'), focus: text('focus').split(',').map(s => s.trim()).filter(Boolean), reservation_conditions: text('constraints') });
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {page === 'research' ? <div className="page-body"><Research projects={projects}/></div> : !workspace ? <div className="page-body">{loading ? <div className="loading"><span className="spinner" />Loading project…</div> : <Empty title="Create your first submission project" body="Start with a posting and the opportunity you want to pursue." action={<button className="button primary" onClick={() => open({ kind: 'project' })}>New project<Plus size={16} /></button>} />}</div> : <div className="page-body">
        ...
        <header className="page-header"><div><h1>{page === 'brief' || page === 'studio' ? project?.name : navigation.find(item => item.id === page)?.name}</h1><p>{page === 'studio' ? `${project?.institution} · Submission studio` : page === 'brief' ? `${project?.institution} · ${project?.domain} application` : page === 'library' ? 'Source-backed facts, cases, and company research across opportunities.' : page === 'evidence' ? 'The facts and sources behind your submission.' : page === 'strategies' ? 'Version the approach. Keep the reasoning behind every change.' : 'Compare strategies against the same hidden selection worlds.'}</p></div>
        ...
        {page === 'library' && <Library projectId={projectId} onImport={() => {void refresh();}} />}
